...
純一はApp Storeのガイドラインを読んでいた。

チュートリアルヘルを抜け出すため、純一が選んだ戦略は「Todoアプリをリリースする」だった。自分で作った唯一のオリジナルアプリ。機能は最小限だが、CRUD操作は動く。UIもデザイナーとして最低限の品質は確保した。データの永続化も、Swiftful Thinkingの動画を参考に@<code>{UserDefaults}で実装した。
...
そして、いざ審査に提出しようとした段階で、純一は検索した。「Todoアプリ　App Store　リリース」。
...
純一はApp Store Connectの画面を閉じた。

五ヶ月間の学習。チュートリアルの写経。自力で作ったTodoアプリ。そのすべてが、App Storeの審査基準という巨大な壁の前で無力だった。

「新しいアプリを、作るしかない」

純一はそう結論づけた。しかし、チュートリアルヘルの只中にいる自分が、ゼロから新しいアプリを設計・実装できるのか。白紙のXcodeプロジェクトの前で固まった記憶が蘇る。

その夜、純一はスマートフォンのメモ帳を開いた。数週間前に書き留めた、あの単語が目に入った。
...
純一がXでブックマークしたツイートのスレッドを辿ると、Connpassにイベントページがあった。「Cursor Meetup Tokyo #3 — AIコードエディタで変わる開発体験」。定員五十名、参加費無料。

会場に入ると、純一は違和感を覚えた。CA.swiftのLT会とは雰囲気が異なる。参加者の半数以上がMacではなくWindowsのノートPCを使っている。iOSエンジニアだけでなく、Web開発者、データサイエンティスト、デザイナーまで、多様なバックグラウンドの人々が集まっている。

イベントが始まった。登壇者がCursorの画面を大きなモニターに映す。

見た目はVisual Studio Codeにそっくりだった。左側にファイルツリー、中央にコードエディタ、下部にターミナル。純一がXcodeで見慣れたIDEのレイアウトと同じだ。
...
純一にとって重要だったのは「質問できる」という点だ。チュートリアルでは得られない「なぜそうなるのか」「自分のコードのどこが間違っているのか」を、リアルタイムで聞ける。
//}

イベントの帰り道、純一はMac App StoreからCursorをダウンロードした。


== 一行の魔法

Cursorをインストールした翌日、純一はもう一つのツールを試してみることにした。

Xのタイムラインで話題になっていた@<b>{Claude Code}——ターミナル上で動作するAIコーディングエージェント。Cursorがエディタ内のAIアシスタントなら、Claude Codeはターミナルから直接コードを生成・編集するツールだ。

純一はClaude Codeをインストールし、空のSwiftUIプロジェクトを作成した。そしてターミナルに、こう入力した。
...
ファイルが自動的に作成されていく。@<code>{TimerView.swift}、@<code>{TimerViewModel.swift}、@<code>{ContentView.swift}。コードが生成され、ファイルに書き込まれていく。純一はターミナルのログを見守った。

三分後。

「完了しました」とClaude Codeが告げた。

純一はXcodeでプロジェクトを開いた。ビルドボタンを押す。
...
純一はそのポモドーロタイマーのコードを読んでみた。@<code>{Timer.publish}で一秒ごとにイベントを発火し、@<code>{.onReceive}でカウントダウンを制御している。@<code>{withAnimation}でプログレスバーがなめらかに動く。コードとしては正しい。しかし——

純一はこのコードを、@<b>{理解していない}。

Claude Codeが生成したコードの中に、純一が知らないAPIがいくつもあった。@<code>{Timer.publish}、@<code>{.onReceive}、@<code>{RunLoop.main}。これらが何をしているのか、純一には説明できない。

「速すぎる」

純一は呟いた。AIにアプリを作ってもらうことは、チュートリアルの写経よりもさらにタチが悪い。チュートリアルの写経は、少なくとも自分の手でコードを打ち込む。AIに丸投げすると、自分の手には何も残らない。
...
純一はCursorを開き、チャットウィンドウに最初の質問を打ち込んだ。
...
純一はCursorの提案に従い、まずデータモデルから手をつけた。Swiftful Thinkingのチュートリアルでは、場所のカテゴリは存在しなかった。しかし鎌倉には寺、神社、そして大仏がある。これらを区別するために、@<code>{enum}を使う。

「enumってどういうときに使うんですか？」と純一はCursorに聞いた。

//quote{
enumは「列挙型」と呼ばれ、決まった選択肢の中から一つを選ぶときに使います。例えば信号の色は「赤・黄・青」の三つだけですよね。これをenumで表現します。鎌倉の場所のカテゴリなら「寺・神社・大仏」の三つです。
//}

純一はCursorの説明を聞きながら、自分の手でコードを書いた。
...
「@<code>{LocalizedString}って、自分で作っていいんですか？」と純一はCursorに聞いた。
...
純一はCursorに質問しては理解し、理解しては自分で書く、というサイクルを繰り返した。Claude Codeのように一発でアプリを生成するのではなく、一行ずつ、対話しながら進める。時間はかかる。しかし、一行ごとに「なぜこう書くのか」を理解できる。

これが、純一にとっての@<b>{AIを先生として使う}ということだった。
...
純一は鎌倉に行ったことがあった。大学時代、友人と日帰りで訪れた記憶がある。鶴岡八幡宮の大階段、小町通りの食べ歩き、大仏の圧倒的な存在感。しかしそのとき純一はカメラでスナップを撮っただけで、寺社仏閣の歴史や文化には興味がなかった。

今は違う。純一はアプリに掲載する寺社仏閣を選定するため、鎌倉の寺社について徹底的に調べた。Wikipediaを読み、公式サイトを訪問し、Google Mapsで座標を確認する。そして週末、実際に鎌倉を訪れて写真を撮影した。

デザイナーの目で、寺社の美しさを切り取る。建長寺の総門の荘厳さ。明月院の丸窓から覗く庭園。報国寺の竹林に差し込む光。高徳院の大仏の静謐な表情。

純一はCursorの助けを借りながら、@<code>{LocationsDataService}にデータを入力していった。
...
各スポットに三枚ずつの写真。純一が実際に鎌倉で撮影した写真だ。デザイナーとしての構図感覚を活かし、建物の正面、ディテール、雰囲気の三種類を撮り分けた。画像はXcodeのAssets.xcassetsに@<code>{imageset}として追加した。

そしてカスタムピンアイコン。純一はFigmaでSVGアイコンをデザインした。寺のアイコンは屋根の曲線を模した形、神社のアイコンは鳥居のシルエット、大仏のアイコンは仏像の輪郭。三種類のピンがカテゴリごとに地図上に表示される。

デザイナーとしてのスキルが、ここで決定的に活きた。エンジニアが作るアプリのアイコンとは、一線を画すクオリティ。純一は自分のバックグラウンドが「弱み」ではなく「強み」だと、改めて実感した。
...
純一はシミュレータの画面をスクリーンショットに撮り、Xに投稿した。
...
純一は鎌倉地図アプリ「Kamakura Temple」をApp Store Connectにアップロードした。
...
純一はXcodeを閉じ、深呼吸した。Appleの審査は通常一日から三日程度で結果が出るという。
...
Todoアプリでリリースを断念したとき、純一が恐れていたあのガイドラインだ。

「Webで検索するのと変わらない機能しかない」——つまり、鎌倉の寺社仏閣の情報を地図上に表示するだけなら、Google Mapsで検索すれば同じことができる。App Storeに並べる価値がない、とAppleは判断した。

純一はMacBook Airの前で、数分間、動けなかった。

五ヶ月以上の学習。鎌倉への取材旅行。十七箇所のデータ入力。カスタムアイコンのデザイン。多言語対応の実装。そのすべてが「不十分」と判定された。

しかし、純一は気づいていた。Appleの指摘は正しい。このアプリには「Webでは得られない独自の体験」が欠けている。地図に情報を表示するだけなら、確かにWebで十分だ。

純一はリジェクトの通知画面を閉じず、もう一度読み返した。

「not sufficiently different from a mobile web browsing experience」。

ネイティブアプリでしかできないこと。Webにはない価値。

純一はそのヒントを握りしめたまま、次の一手を探し始めた。


== エヴァンジェリストの助言

リジェクトの翌週、純一はConnpassで「Apple Technology Day」というイベントを見つけた。

Appleが定期的に開催するエンジニア向けのイベントで、Appleのエヴァンジェリストやエンジニアが参加者の質問に直接答えてくれるという。純一は迷わず参加登録した。

会場はAppleの日本法人が入るビル。受付を済ませると、広いイベントスペースにテーブルが並び、Appleのスタッフが各テーブルで開発者の相談を受けている。

純一は一人のスタッフの前に座った。名札には「石武」と書かれている。Appleのデベロッパーリレーションズのエヴァンジェリストだった。

「あの、App Store審査で4.2 Minimum Functionalityでリジェクトされたんですが——」

純一は事情を説明した。鎌倉の寺社仏閣の地図アプリ。十七箇所のスポット。カテゴリ別のカスタムピン。多言語対応。しかし「Webで検索するのと変わらない」と判定された。
...
純一はイベント会場を出ると、走るようにして最寄りのカフェに入った。MacBook Airを開き、Cursorを起動する。チャットウィンドウに入力した。
...
まず、コメントのデータモデルを設計する。コメントには何が必要か。テキスト、座標、タイムスタンプ、ユーザー情報。Cursorに相談しながら、純一は@<code>{Comment}構造体を書いた。
...
ここで純一は壁にぶつかった。@<code>{CLLocationCoordinate2D}は@<code>{Codable}に準拠していない。つまり、そのままではJSONにエンコード・デコードできず、データの保存ができない。
...
Cursorの説明に従い、純一は@<code>{CodingKeys}とカスタムの@<code>{encode}/decode}メソッドを実装した。
...
純一はこのコードを書きながら、@<code>{Codable}プロトコルの仕組みを深く理解した。Cursorに質問し、説明を受け、自分で書く。このサイクルが、純一の学習を加速させていた。
...
一週間の実装期間。Cursorとの対話は数百回に及んだ。しかし、すべてのコードは純一が自分の手で書いた。AIが生成したコードをコピー＆ペーストしたのではなく、AIに教わりながら、一行ずつ理解して書いた。
...
純一はコメント機能を追加した「Kamakura Temple」の新バージョンをApp Store Connectにアップロードした。

審査チームへのメモ欄に、純一は以下を記載した。
...
二日目。通知なし。純一はXcodeを開いては閉じ、App Store Connectのステータスを何度も確認した。「In Review」の文字が変わらない。
...
純一のアプリは最初のバージョンがリジェクトされ、コメント機能を追加した二回目の提出で承認された。リジェクトは失敗ではなく、アプリを改善するためのフィードバックだ。
//}

純一はApp Storeを開いた。検索バーに「Kamakura Temple」と入力する。
...
今、App Storeに、純一の名前が載っている。

@<b>{Developer: Junichi Mizuno}
...
見知らぬ誰かが、App Storeで「Kamakura Temple」を見つけ、スクリーンショットを見て、ダウンロードボタンを押した。その人のiPhoneに、純一が書いたコードが動いている。純一がデザインしたピンアイコンが、鎌倉の地図上に表示されている。

純一はしばらく、通知画面を見つめていた。

一ダウンロード。数字としては微々たるものだ。しかし、その一ダウンロードは、純一にとって、デザイン会社で作ったどんなパンフレットよりも重い意味を持っていた。

翌週の村田さんのもくもく会。純一はApp Storeの画面をみんなに見せた。
...
//quote{
@mizuno_junichi リリースおめでとうございます！6ヶ月でここまで来たのは本当にすごい。しかもリジェクト対応も経験したのは大きい。実務でもApp Store審査とのやりとりは避けて通れないから、ここで経験できたのは財産ですよ。次のアプリも楽しみにしてます！
//}

中野からもDMが来た。

//quote{
純一くん、アプリリリースおめでとう！Ignoreのアイコン作ってくれたときは何もできなかったのに、半年で自分のアプリ出すとか成長早すぎない？笑
//}

純一は中野のメッセージを読み返した。半年前、コワーキングスペースでアイコンを作ることしかできなかった自分。XcodeもSwiftも知らなかった自分。

今、App Storeに自分のアプリが並んでいる。

純一はXcodeを開いた。次にやることは、もう決まっていた。

一つ目は、LT会で発表すること。CA.swiftで「いつかあのステージで発表する」と心に決めたあの夜。自分のアプリがある今なら、発表できるネタがある。リジェクトされて、改善して、リリースした体験談。初心者にも参考になるはずだ。

二つ目は、二つ目のアプリを作ること。Kamakura Templeは純一の最初のリリースアプリだが、チュートリアルのコードをベースにしている。次は、完全にゼロからオリジナルのアプリを作りたい。
...
村田さんが最近、もくもく会の会場にしている下北沢のカフェスペースで「間借りカフェ」を始めたいと話していた。もくもく会の頻度は変わるかもしれない。しかし純一は、コミュニティとのつながりを大切にしたいと思っていた。

午前一時。

純一はApp Storeの「Kamakura Temple」のページを開いた。ダウンロード数は十二。レビューはまだゼロ。売上もゼロ。

しかし、その画面は純一にとって、星空のように見えた。十七のピンが鎌倉の地図に散りばめられている。一つ一つが、純一が調べ、撮影し、コードを書いた場所だ。

純一はMacBook Airを閉じた。六ヶ月前、このMacBook Airは冷たい光を放つだけの機械だった。今は、純一の作品を世界に届ける道具に変わった。

明日は、LT会の登壇申し込みをする。

純一はベッドに入り、目を閉じた。瞼の裏に、鎌倉大仏の穏やかな表情が浮かんだ。高徳院の境内で撮影した写真。コードの中では@<code>{CLLocationCoordinate2D(latitude: 35.3169, longitude: 139.5358)}という座標に過ぎないが、純一にとっては自分のアプリの、自分の旅の、記憶だった。
