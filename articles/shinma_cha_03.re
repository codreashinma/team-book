...
　@<tt>{mapping}。辞書型。連想配列。JavaScriptでいう@<tt>{Map}やオブジェクトのようなもの。@<tt>{address}をキーにして、@<tt>{uint256}——符号なし二五六ビット整数——を値として持つ。

　蓮は三秒かけて、その意味を咀嚼した。

　「ウォレットアドレス → 残高」。

　J-Coinの残高は、この一行で定義されていた。銀行のメインフレームが何百万行ものCOBOLで管理しているはずの「誰がいくら持っているか」という情報が、ブロックチェーンの上ではたった一行の@<tt>{mapping}に収まっている。
...
　昨日、@<tt>{balanceOf}で返ってきた「100000000000000000000」の正体。あの十八桁のゼロの群れは、ここから来ていたのだ。ブロックチェーン上のトークンは整数しか扱えない。小数点を使えない代わりに、「小数点以下十八桁」という精度をこの一行で宣言している。100 J-Coinは、内部では100に10の18乗を掛けた巨大な整数として記録される。
...
　残高照会。アドレスを渡すと、@<tt>{_balances}の表から対応する数値を返す。それだけだ。ATMに表示される「お預かり残高」の裏側にあるものが、たった三行の関数だった。
...
　蓮は一行ずつ読んだ。@<tt>{msg.sender}——この関数を呼び出した人のアドレス。@<tt>{require}で残高不足をチェック。送り手の残高を減らし、受け手の残高を増やす。@<tt>{Transfer}イベントを発行して記録を残す。@<tt>{true}を返す。

　蓮はモニターから目を離し、自分の両手を見下ろした。

　昨日、テストネットで「送金」ボタンを押したとき、裏側ではこのコードが動いていた。自分の@<tt>{_balances}から10が引かれ、Account 2の@<tt>{_balances}に10が足された。それが「送金」の全貌だった。
...
　@<tt>{approve}。直訳すれば「承認する」。自分のトークンを、別のアドレスが代わりに動かすことを許可する関数。そしてこの@<tt>{approve}とセットで存在する@<tt>{transferFrom}——許可された第三者が、本人に代わってトークンを移動させる。

　なぜ、そんなものが必要なのだ？

　自分で@<tt>{transfer}すれば済む話ではないのか。ドキュメントを読み返したが、ユースケースの具体例が見つからない。技術的な仕様は書いてあるのに、「なぜ」が書かれていない。仕様書とはそういうものだ。Whatは語るが、Whyは語らない。

　蓮はDiscordを開いた。今回は@<tt>{#sdk-help}ではなく、kirishi_maに直接DMを送った。二日前のやり取りで、質問を送るくらいの距離感は許されると感じていた。

//quote{
@<b>{fujisaki_ren}: すみません、また質問です。ERC-20の@<tt>{approve}と@<tt>{transferFrom}の仕組みは理解したんですが、なぜ必要なのかが分からなくて。@<tt>{transfer}で十分では？
//}

　返事は一分で来た。

//quote{
@<b>{kirishi_ma}: いい質問。@<tt>{transfer}は「自分の手で渡す」。@<tt>{approve} + @<tt>{transferFrom}は「引き落とし」。

自動販売機を想像して。あなたが自販機にお金を入れる。それが@<tt>{approve}。自販機がジュースの代金ぶんだけ取る。それが@<tt>{transferFrom}。あなたがいちいち「120円送金」しなくても、自販機が必要な額を自動で引き出せる。
...
　@<tt>{transfer}は人間が手動で呼ぶ関数だ。@<tt>{approve} + @<tt>{transferFrom}は、プログラムが自動で呼ぶ関数。つまりERC-20は最初から、人間だけでなく「コードがお金を動かす」ことを前提に設計されている。お金がプログラマブルである——その意味が、ようやく腑に落ちた。

//quote{
@<b>{fujisaki_ren}: なるほど。ERC-20はお金の仕様書であると同時に、プログラムとお金を接続するインターフェースでもある、ということですか。
//}

//quote{
@<b>{kirishi_ma}: そういうこと。だからERC-20は「トークン標準」と呼ばれる。全員が同じインターフェースに従うから、どのトークンでも同じコードで扱える。J-CoinもUSDCもDAIも、@<tt>{transfer}の呼び方は同じ。
//}

　蓮はエンジニアとしての直感で、その設計の力を理解した。インターフェースの標準化。REST APIが世界に普及した理由と同じだ。全員が同じ約束事に従うから、組み合わせが爆発的に広がる。

//quote{
@<b>{kirishi_ma}: ところで、コントラクトのコード読んだんだ？
//}

//quote{
@<b>{fujisaki_ren}: はい、PolygonScanで。
//}
...
//quote{
@<b>{fujisaki_ren}: 怖かった。こんな短いコードがお金を定義していることが。
//}

　既読がついてから、返信が来るまで三十秒の間があった。kirishi_maにしては長い。

//quote{
@<b>{kirishi_ma}: その感覚は正しい。
...
//quote{
@<b>{fujisaki_ren}: SDKが「ガードレール」のようなものですか。
//}

//quote{
@<b>{kirishi_ma}: 近い。ただ、もっと積極的な意味もある。
...
　蓮はノートを見下ろした。左ページには今日学んだ技術のメモ。ERC-20の関数たち、@<tt>{mapping}の構造、@<tt>{approve}と@<tt>{transferFrom}のパターン。右ページには、たった今書き取ったばかりの法律のメモ。電子決済手段、信託保全、資金決済法。
...
//quote{
@<b>{fujisaki_ren}: さっき丘部さんの配信を見ました。技術と法律が表裏一体という話、すごく腑に落ちました。
//}

//quote{
@<b>{kirishi_ma}: あの配信見たんだ。丘部さんの話はいつ聞いてもブレない。
//}

//quote{
@<b>{fujisaki_ren}: 一つ聞いてもいいですか。霧島さんはなぜジェイコイン・ラボに？
//}

　踏み込みすぎたかと、送信した直後に思った。でも返事はすぐに来た。

//quote{
@<b>{kirishi_ma}: Ethereumの初期からスマートコントラクトを書いてきて、ずっと思ってたことがある。この技術は、投機じゃなくて実用に使われるべきだと。NFTバブルもDeFiの熱狂も見てきた。でも「普通の人が普通に使えるもの」は、なかなか生まれなかった。丘部さんのビジョンを聞いたとき、この人は本気で「普通の人」を見てると思った。
//}

//quote{
@<b>{fujisaki_ren}: ……僕もそう思いました。ホワイトペーパーを読んだとき。
//}

//quote{
@<b>{kirishi_ma}: 君の商店街の決済アプリ、まさにその方向だと思う。進捗あったら聞かせて。
//}

　蓮は「ありがとうございます」と返して、Discordを閉じた。

　モニターにはまだ、J-Coinのスマートコントラクトが表示されていた。七十八行。@<tt>{mapping}と@<tt>{function}と@<tt>{require}で組み上げられた、お金の骨格。
