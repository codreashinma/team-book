...
まずSlackを開いた。チャンネル一覧。 `#dev-general` 、 `#dev-backend` 、 `#dev-infra` 、 `#dev-frontend` 。他に `#random` 、 `#lunch` 、 `#tech-news` 。標準的なSlackの構成。
...
ドキュメントを探した。`docs/` ディレクトリは存在しない。コメントを探した。ほとんどない。あるのはコードだけだ。十万行のコードだけが、前任者の全てを物語っていた。
