...
純一はその第一話を再生した瞬間から、Todoアプリとはまったく異なる世界が広がることを直感した。Todoアプリは「リストにテキストを並べる」UIだった。地図アプリは違う。現実世界の空間を、コードで操作する。
...
純一はプレビューを見た。MacBook Airの画面に、東京駅周辺の地図が表示されている。道路、建物、駅名——すべてがリアルタイムにレンダリングされている。指でピンチすればズームし、ドラッグすればスクロールする。

たった二十行程度のコードで、地図アプリの基盤ができた。

「これ、Googleマップみたいなことができるってこと？」

純一は思わず声に出した。もちろんGoogleマップほどの機能は実装できないが、基本的な地図表示、ピンの配置、ユーザーの現在地追跡、経路検索——すべてMapKitに含まれている。Appleが無料で提供しているフレームワークだ。
...
純一はこの型名を見て、デザイナーとしての記憶が蘇った。Illustratorでオブジェクトを配置するとき、X座標とY座標を指定する。それとまったく同じだ。ただし、Illustratorの座標系はアートボードの左上が原点で、右にX、下にYが増える。地球の座標系は、赤道が緯度ゼロ、グリニッジ子午線が経度ゼロ。スケールは違うが、「二つの数値で位置を決める」という本質は同じだった。
...
純一はプレビューを更新した。地図上に青い丸アイコンが表示され、その下に逆三角形のしっぽが付いている。まるで、Google Mapsのピンのような外観だ。しかしこれは、純一が——SwiftUIのコードとSF Symbolsだけで作った、完全にカスタムのピンだ。

デザイナーの血が騒いだ。

「色を変えよう。サイズも調整したい。影も付けたい」

純一はモディファイアを次々に追加していった。@<code>{.shadow()}、@<code>{.scaleEffect()}、@<code>{.animation()}。第一章で学んだモディファイアの知識が、ここで活きた。Todoアプリで覚えた技術が、地図アプリという新しい文脈で再利用できる。これがプログラミングの醍醐味なのだと、純一は体で理解し始めていた。
...
場所のデータを管理する@<code>{ViewModel}の設計。地図のカメラをアニメーション付きで移動させる方法。タップした場所の詳細情報をシート表示する実装。純一はNickの動画を一つずつ追いかけ、コードを写経した。

特に純一の目を引いたのは、地図のカメラを特定の場所にアニメーション付きで移動させるコードだった。
...
純一はこのチュートリアルを通じて、SwiftUIの@<b>{MVVMアーキテクチャ}に初めて触れた。
...
純一はここで「データの流れ」を意識し始めた。第一章のTodoアプリでは@<code>{@State}ですべてを管理していたが、アプリが複雑になると、状態管理のロジックをViewの外に出す必要がある。それがViewModelの役割だ。

チュートリアルの地図アプリを完成させるのに、純一は二週間かかった。毎晩二時間、昼休みの三十分、通勤電車の往復一時間。合計すると四十時間近い学習時間だ。完成した地図アプリは、複数の観光地をリスト表示し、タップするとアニメーション付きで地図が移動し、詳細情報がシートで表示される。第一章のTodoアプリとは比べものにならない複雑さだった。

純一はiPhoneのシミュレータで地図アプリを動かしながら、ある衝動を感じた。
...
純一はSwiftUIの学習を続けるうちに、ある異変に気づいた。
...
純一はその場でスマートフォンを取り出し、Amazonで検索した。「SwiftUI」で絞り込む。結果は同じだった。最新のものでも二〇二〇年。それ以降、SwiftUI専門の書籍はほとんど出版されていない。
 ...
 * 「SwiftUI Tutorials」：第一章で純一が取り組んだLandmarksチュートリアル。
 * 「WWDC Sessions」：毎年のWWDCで発表されるセッション動画。新機能の解説が最も正確かつ詳細に語られる。

@<b>{2. 海外のYouTubeチャンネル}
 * Swiftful Thinking：純一が使っているチャンネル。体系的なブートキャンプ形式。
...
純一は「The Swift Programming Language」のページをブックマークした。英語だが、翻訳ツールを併用すれば読める。Swiftful Thinkingで手を動かし、疑問が出たら公式ドキュメントで確認する。そしてSwift言語の基礎を体系的に理解するために「Swift実践入門」も注文した。

書籍に頼れないという事実は、裏を返せば、@<b>{最新の情報に誰でもアクセスできる}ということでもあった。高額な専門書を買い揃えなくても、Apple公式のドキュメントとYouTubeがあれば学べる。純一のようなキャリアチェンジ組にとって、これは大きなアドバンテージだった。


== 一四〇文字の開発日記

ある夜、純一はシミュレータに映る地図アプリのスクリーンショットを撮った。

東京タワー、浅草寺、渋谷スクランブル交差点——チュートリアルのサンプルデータを日本の観光地に置き換えたものだ。地図上にカスタムのピンが立ち、タップするとシートが開いて詳細情報が表示される。

純一はXの投稿画面を開いた。第一章で志村さんに勧められて以来、学習の進捗をXに投稿する習慣がついていた。最初は恥ずかしかったが、志村さんの「アウトプットすると記憶に定着する」という言葉を信じて続けていた。
...
//quote{
@mizuno_junichi 25日でここまで来たのすごいですね。MVVMを意識できているのがいい。次はObservableマクロ（@Observable）も調べてみてください。@StateObjectや@ObservedObjectを使わずにもっとシンプルに書けるようになります。iOS 17以降の新しい書き方です。
//}

他にも反応があった。フォロワー二十人のアカウントが、五十人に増えていた。同じようにSwiftUIを学んでいる人、個人開発をしている人、現役のiOSエンジニア。純一の投稿に「いいね」や「リポスト」をしてくれる人が少しずつ増えている。

特に嬉しかったのは、純一と同じ時期にSwiftUIの学習を始めたらしいアカウントからのリプライだった。

//quote{
@mizuno_junichi 自分もSwiftful Thinkingで勉強中です！地図アプリのチュートリアルちょうどやるところだったので参考になります。お互い頑張りましょう！
//}

一人じゃない。

純一はその事実に、想像以上の力をもらった。デザイン会社のアシスタントとして、上司とクライアントの間で修正データを作る日々。その孤独な作業と対照的に、Xには同じ道を歩む仲間がいた。

純一はこの習慣に名前があることを、後になって知った。
...
個人開発において、Build in Publicは「マーケティング」と「学習の記録」と「コミュニティ形成」を同時に行える強力な手法だ。純一が何気なく始めたXでの学習投稿は、まさにBuild in Publicそのものだった。
//}

純一はそれまで、デザインの仕事でSNSを「作品発表の場」として使ったことはあった。Behanceにポートフォリオを上げ、完成した美しい成果物だけを見せる。しかしBuild in Publicは違う。未完成でもいい。バグだらけでもいい。むしろ、@<b>{過程を見せること}に価値がある。

この考え方は、デザイン業界の「完成品だけを見せる」文化とは真逆だった。そして純一は、こちらの方が自分に合っていると感じた。
...
純一がSwiftUIの学習を始めて一ヶ月半が過ぎた。Xで学習記録を投稿し続ける中で、ある単語が頻繁にタイムラインに流れてくることに気づいた。

@<b>{Connpass}。

エンジニア向けのイベント・勉強会のプラットフォームらしい。「もくもく会」「LT会」「ハンズオン」——さまざまなイベントが毎日のように開催されている。純一はアカウントを作成し、「iOS」「SwiftUI」「個人開発」で検索してみた。

そこで見つけたのが、「個人開発者もくもく会」だった。

主催者は村田さんという方。プロフィールには「個人開発歴五年、リリースアプリ十二本」とある。毎月第二土曜日に、都内のカフェスペースで開催している。参加費は五百円（ドリンク代）。定員十名。

純一は迷わず参加ボタンを押した。中野と美咲のもくもく会に参加したときのことを思い出す。あのとき純一はデザイナーとして参加し、アイコンを作ることしかできなかった。今は違う。自分のXcodeプロジェクトを持っている。地図アプリのチュートリアルも完走した。
...
「水野です。グラフィックデザイナーで、今SwiftUIを勉強しています。まだ一ヶ月半くらいなんですけど」
...
純一は苦笑した。まだ「SwiftUIを書ける」と胸を張れるレベルではない。しかし、デザイナーとしての経験が「強み」として評価されるのは純粋に嬉しかった。デザイン会社では「アシスタント」という肩書が自分の価値のすべてだった。ここでは「デザインができる」というスキルが、別の意味を持つ。

もくもく会が始まると、各自がそれぞれのプロジェクトに取り組んだ。純一はSwiftful Thinkingの次のチュートリアルを進めようとしたが、周りの参加者の画面が気になった。

隣の席の女性は、植物の水やりリマインダーアプリを開発していた。ローカル通知とCore Dataを使っているらしい。向かいの男性は、カメラアプリに独自のフィルターを実装中。Core Imageというフレームワークを使うのだという。

純一が知らないフレームワーク、知らない技術が、ここにはごろごろ転がっていた。

休憩時間、村田さんが純一に話しかけてきた。

「水野さん、Xで学習記録つけてるんでしょ？　見ましたよ。いいですね、あのペース。ところで、来週CA.swiftっていうLT会があるんだけど、興味ない？」

「LT会？」

「ライトニングトークの略。五分とか十分の短いプレゼンを何人かが順番にやるイベント。大手のIT企業が主催してて、プロのiOSエンジニアが発表するやつ」

純一の心臓が跳ねた。プロのiOSエンジニアの話を直接聞ける機会。
...
「CA.swift #22 〜Swiftの進化を活かした技術基盤への挑戦〜」と題されたLT会の会場は、純一が想像していたよりもはるかに洗練されていた。

広いイベントスペースにモニターが並び、百人以上のエンジニアが着席している。純一は後方の席に座った。村田さんが隣に来てくれたので、少しだけ安心した。
...
@<code>{async}、@<code>{await}、@<code>{Task}、@<code>{throws}、@<code>{try}——純一にとってはほぼすべてが初見の単語だった。
...
純一はメモ帳アプリを開き、知らない単語を片っ端から書き留めた。
...
十五個以上の未知の技術用語。純一が知っているのは、@<code>{VStack}と@<code>{@State}と@<code>{List}くらいだ。自分が立っている場所と、プロのiOSエンジニアが立っている場所の距離を、純一はこの夜、初めて正確に測った。
...
純一が今すぐ理解する必要はないが、@<b>{APIからデータを取得する}（ネットワーク通信）はほぼすべてのアプリで必要な機能であり、その実装にはasync/awaitが使われる。将来、必ず向き合うことになる技術だ。
//}

LT会の後、懇親会があった。ビールとピザが振る舞われ、エンジニアたちが技術談義に花を咲かせている。純一は壁際で炭酸水を飲んでいた。六本木の言語交換ミートアップのときと同じだ。壁際の男。

しかし今回は違った。六本木のときは「何者でもない自分」だったが、今は「SwiftUIを学んでいる自分」がいる。小さくても、語れるものがある。

純一は勇気を出して、近くにいたエンジニアに話しかけた。
...
純一はまた一つ、知らない単語を仕入れた。UIKit。Storyboard。SwiftUIの前の世界。

会場を出るとき、純一は空を見上げた。十二月の夜空は澄んでいて、星はほとんど見えないが、ビルの灯りが代わりに光っている。

「いつか、あのステージで発表する」

純一は誰にも聞こえない声で呟いた。それは決意というよりも、予感だった。


== バージョン管理という地図

翌週のもくもく会。純一がコードを書いていると、村田さんが画面を覗き込んできた。
...
純一はその場で、GitHubアカウントを作成した。ユーザー名は「mizuno-junichi」。プロフィール写真には、自分がデザインしたIgnoreのアイコンを設定した。
...
「空のGitリポジトリが初期化されました」。たった一行のメッセージが、純一にとっては新しい章の始まりだった。

@<code>{git add .}、@<code>{git commit -m "最初のコミット"}、@<code>{git push -u origin main}。

コマンドを一つずつ打ち込むたびに、純一は自分が「エンジニアの道具」を一つずつ手に入れている実感を覚えた。Xcode、SwiftUI、MapKit、そしてGit。デザイナーがIllustrator、Photoshop、InDesign、Bridgeと道具を揃えていくのと同じように。
...
十二月も半ばを過ぎ、純一のSwiftUI学習は三つの柱で進んでいた。
...
三つを並行して進める中で、純一は@<b>{Optional}という概念にぶつかった。
...
純一は最初、この概念が理解できなかった。変数には値が入っているか入っていないかの二択で、なぜわざわざ「入っていないかもしれない」を型として表現するのか。
...
純一はこれらのパターンをノートに書き出した。デザイナー時代のスケッチブックに、今はSwiftのコードパターンが書かれている。
...
純一はXcodeで「New Project」を選択し、空のSwiftUIプロジェクトを作成した。Product Nameに「MyOriginalApp」と入力する。
...
チュートリアルのときは、Nickが「ここに@<code>{List}を追加しましょう」と言ってくれた。Apple公式チュートリアルは、次に書くべきコードがステップバイステップで提示されていた。しかし今、画面には何のガイドもない。白紙のXcodeプロジェクト。「Hello, World!」だけが、純一を見つめ返している。

三十分が経った。純一が書いたのは、@<code>{Text("Hello, World!")}を削除して@<code>{Text("")}にしただけだった。

一時間。画面は変わらない。

純一は席を立ち、コーヒーを淹れた。マグカップを両手で包みながら、MacBook Airの画面を遠くから見つめた。
...
純一はXの検索結果を読み漁った。同じ苦しみを語る人が、驚くほど多かった。
...
純一はその言葉を噛みしめた。デザインの世界にも同じ構造がある。デザインの教科書を読み、他人の作品を模写し、ソフトの操作を覚える。しかし、白紙のアートボードにゼロからデザインを生み出す力は、模写だけでは身につかない。自分で考え、自分で判断し、自分で形にする経験が必要だ。

純一はコーヒーを飲み干し、Xcodeの前に座り直した。


== 白紙のXcodeプロジェクト

チュートリアルヘルを自覚した純一は、戦略を変えることにした。
...
純一は第一章で作ったTodoアプリの画面をシミュレータに表示した。自分が一から設計した、唯一のオリジナルアプリ。シンプルだが、CRUDは動く。UIもデザイナーとして最低限の品質はある。
...
純一はXcodeのMyTodoプロジェクトを開き直した。白紙のプロジェクトではなく、すでに動いているプロジェクトだ。ここに機能を追加し、改善し、リリースする。

しかし、純一のコードの前に立ちはだかる壁があった。

志村さんが以前リプライで言っていた「データの永続化」。アプリを閉じてもデータが残る仕組み。純一はSwiftful Thinkingで@<code>{UserDefaults}と@<code>{SwiftData}の動画を見たが、自分のTodoアプリにどう組み込むかがわからない。チュートリアルでは「こう書いてください」と言われた通りに書けば動いた。しかし、自分のコードのどこに、どう組み込めばいいのか。

設計の力が足りない。

純一はXcodeの画面を見つめた。白紙ではないが、次の一行が書けない。

「……誰かに聞けたらいいのに」

純一は呟いた。志村さんはXでアドバイスをくれるが、コードの詳細まで見てもらうのは限界がある。村田さんはもくもく会で相談に乗ってくれるが、月に一度しか会えない。

その夜、純一はXのタイムラインを眺めていた。とあるツイートが目に入った。

//quote{
AIコードエディタ「Cursor」のイベントに行ってきた。すごい時代になったな。AIがペアプログラミングの相棒になる日が来るとは。
//}

AIコードエディタ。純一はそのツイートをブックマークした。

午前一時。MacBook Airの画面には、Todoアプリのコードが表示されている。純一はまだ「次の一行」を見つけられていない。しかし、チュートリアルヘルの自覚は、純一にとって大きな一歩だった。

問題が見えている。問題が見えているなら、解き方も見つかるはずだ。

デザインの世界で、純一はそうやって壁を乗り越えてきた。クライアントの曖昧な要望を「問題」として定義し、視覚的な「解決策」に変換する。それがグラフィックデザイナーの仕事だ。
...
GitHubのリポジトリに、純一の決意がコミットされた。

まだ何もリリースしていない。まだチュートリアルヘルの只中にいる。しかし、純一のGitHubには八つのリポジトリが並び、Xのタイムラインには四ヶ月分の学習記録が刻まれ、Connpassには参加イベントの履歴が残っている。

積み重ねてきたものは、消えない。コードと同じだ。@<code>{git log}を叩けば、すべての履歴が表示される。

純一のMacBook Airが、スリープに入る前の最後の光を放った。画面が暗くなる直前、純一はブックマークしたツイートをもう一度見た。

「AIコードエディタ、Cursor——」

その名前を、スマートフォンのメモ帳に書き留めた。Swift、SwiftUI、Xcode、MapKit、Git、GitHub。そしてCursor。

純一の道具箱に、新しい名前が加わろうとしていた。
