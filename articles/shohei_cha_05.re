...
翔はモバイルアプリ企業が主催するLT会に参加していた。会場は六本木のオフィスビル。百名以上のiOSエンジニアが集まっている。

四人目の登壇者は、大手メガベンチャーでiOSチームのリードを務めるエンジニアだった。テーマは「iOS開発チームの採用戦略 2025」。

翔は、そのスライドの一枚に釘付けになった。
...
翔は自分のポケットの中のiPhoneを握りしめた。そのiPhoneには、Cursorがインストールされている。AIコードエディタ。翔がiOSエンジニアになるために最も頼りにしてきたツール。

そのAIが、翔がiOSエンジニアとして就職する道を塞いでいる。
...
LT会の帰り道、翔は六本木の交差点に立ち、空を見上げた。

今まで、AIは翔にとって@<b>{最高の先生}だった。Cursorがなければ、鎌倉地図アプリもSwiftLingoも完成しなかった。AIのおかげでチュートリアルヘルを抜け出し、リジェクト対応もでき、RevenueCatの実装にも挑戦できた。

その同じAIが、今度は翔の就職の@<b>{最大の障壁}になろうとしている。

ゲームに例えるなら、AIは味方のNPCから@<b>{ラスボス}に変わったのだ。

しかし翔は、ラスボスの攻略法を知っている。このラスボスは「AIがあるからジュニアは要らない」と言う。ならば、翔は「AIを使いこなせるジュニア」になればいい。AIと戦うのではなく、AIを武器にして戦う。
...
翌日、翔はMacBook Airを開き、三つのサービスに登録した。
...
翔はLaprasのプロフィールを開いた。自動収集された情報が並んでいる。

GitHubのリポジトリが八つ。Connpassのイベント参加履歴が十二件。もくもく会の主催が六回。LT登壇が一回。これらがスコアに反映されている。

Haruのアドバイスを思い出した。「イベントに参加するとスコアが高くなる」——翔が一年間続けてきたコミュニティ活動が、数値として可視化されている。

次に、履歴書の作成。翔はFigmaを開いた。

デザイナーとしての経験が、ここで決定的に活きた。一般的なエンジニアの履歴書はWordやGoogleドキュメントで作られるテキスト主体のものだ。翔はFigmaで、視覚的に美しい履歴書を作った。
...
「グラフィックデザイナーのスキルを活かし、UIデザインから実装まで一貫して行えます」——翔の強みを一行で表現した。

履歴書のPDFを出力し、Wantedlyのプロフィールにもポートフォリオを添付した。Findyのスキル偏差値を確認する。GitHubの活動頻度と、Connpassの実績が反映されている。

翔は応募を始めた。Wantedlyの「話を聞きに行きたい」ボタンを押し、Findyの「興味がある」をタップし、Laprasのスカウトを待つ。
...
翔はLT会で鍛えたプレゼンスキルを活かし、簡潔に自己紹介した。グラフィックデザイナーとしての経歴、iOSアプリ開発との出会い、二つのアプリのリリース。
...
翔は答えた。「Swiftful Thinkingのチュートリアルで学んだアーキテクチャがMVVMだったので、それをベースにしました」

面接官の表情が微かに曇った。「チュートリアルで学んだから、ですか。MVVMのメリットを、他のアーキテクチャと比較して説明できますか？　例えばMVCとの違いは？」

翔は口ごもった。MVCという言葉は聞いたことがある。Model-View-Controller。しかし、MVVMとの具体的な違いを説明できるほどの理解はなかった。
...
質問が進むたびに、翔の声は小さくなっていった。

面接が終わった後、翔はMacBook Airの画面を閉じ、両手で顔を覆った。

何も答えられなかった。

アプリは二つリリースした。LT会で登壇した。もくもく会を主催した。しかし、企業が求める技術知識——MVC、RxSwift、Combine、テストコード、CI/CD、コードレビュー——翔はその@<b>{どれも}経験がなかった。
...
翔はNotionを開き、新しいページを作成した。タイトルは「@<b>{技術面接 問答集}」。
...
翔はこのコードを書きながら理解した。自分がSwiftUIで書いてきた@<code>{@State}の仕組みは、「宣言的UI」の根幹だったのだ。状態を変えればUIが自動で更新される。それがSwiftUIの設計思想だった。
...
翔はCursorに質問した。「なぜSwiftUIのViewはStructなんですか？　Classじゃダメなんですか？」
...
問答集は日を追うごとに項目が増えていった。「Genericsとは何か」「クロージャとは何か」「ARCとは何か」「プロトコル指向プログラミングとは」「Swift Concurrencyの@<code>{actor}とは」——クラフタの面接で答えられなかった質問、そしてこれから聞かれるであろう質問を、翔はCursorと対話しながら一つずつ潰していった。


== 師匠の言葉

十月中旬。もくもく会で、翔はHaruに相談した。

「面接で全然答えられなくて……技術的な知識が足りないのはわかってるんですが、問答集を作っても、本当にそれで面接に通るのか不安で」

Haruは少し考えてから言った。

「翔さん、一人紹介したい人がいるんですけど。自分の師匠みたいな人で、山崎さんっていうベテランのiOSエンジニアなんですが。大手IT企業で十年以上やってて、採用面接の面接官もやってる人です。技術面接のアドバイスもらえると思います」

翌週、翔はHaruの紹介で山崎さんとオンラインで話す機会を得た。

山崎さんは穏やかだが鋭い目をした人だった。翔の履歴書とポートフォリオを事前に見てくれていた。

「雪川さん、まずポートフォリオは非常にいいです。デザイナーのバックグラウンドが活きていて、他のジュニア候補者と明確に差別化されています。アプリも二つリリースしている。これは実行力の証明になります」
...
山崎さんは続けた。「それから、雪川さんの場合、AIを使いこなしている点は@<b>{強調すべき}です。CursorやClaude Codeを使って個人開発している経験は、今の採用市場ではプラスです。『AIに丸投げしているのではなく、AIを先生として対話しながら学習・開発している』というスタンスを伝えてください」
...
「雪川さん、デザイナーからエンジニアへの転身は、@<b>{希少な経験}です。コードを書けるデザイナーはいますが、App Storeにアプリをリリースしたデザイナーは少ない。その希少性を武器にしてください」
...
翔は正直に答えた。「書籍は『Swift実践入門』を読みました。ただ、Swiftは言語仕様の変更が速く、2020年以降は書籍の出版が追いつけていない状況だと理解しています。そのため、Apple公式のThe Swift Programming LanguageやSwiftUI公式ドキュメント、海外のYouTubeチャンネル（Swiftful Thinking）、そしてCursorを活用して学習しています」

面接を重ねるごとに、翔の回答は洗練されていった。最初のクラフタの面接では何も答えられなかった。しかし、問答集を作り、Cursorに質問し、山崎さんにアドバイスをもらい、実際の面接で経験を積むことで、翔の技術面接力は確実に向上していた。

二次面接に進めたのは@<b>{十五社}。不採用通知が毎週のように届いた。

しかし翔は、応募を止めなかった。もくもく会を一人でも開催したように、就活も一社に落ちても次に応募した。Build in Public——翔はXに就活の状況も投稿していた。
...
就活と並行して、翔は三つ目のアプリを開発していた。

SwiftLingoの英語学習アプリを作る中で、翔はiOSエンジニアにとって英単語の知識が重要であることを痛感していた。APIのドキュメント、エラーメッセージ、Stack Overflowの回答——すべてが英語だ。
...
そして今回、翔は@<b>{AdMob}による広告収入に挑戦する。Shipatonで入れられなかったRevenueCatのサブスクリプション課金とは異なるアプローチだ。Haruが「個人開発の収益化はまずAdMobから始めるのがおすすめ」と言っていた。
...
@<code>{Info.plist}にAdMobのアプリIDを設定し、App Transport Securityの例外設定も追加する。翔はCursorに確認しながら、一つずつ設定を進めた。
...
「@<code>{UIViewRepresentable}って何ですか？」と翔はCursorに聞いた。
...
翔はここで初めて、UIKitとSwiftUIの関係を実感した。面接で聞かれた「UIKitは使ったことありますか？」という質問。当時は「ありません」としか答えられなかったが、AdMobの実装を通じて、UIKitのビューをSwiftUIに統合する方法を学んだ。
...
@<code>{[weak self]}——面接で聞かれそうな概念だ。翔はCursorに質問した。
...
翔は開発を進めながら、面接対策を同時にこなしていた。コードを書くたびに新しい技術概念に出会い、それが面接の回答力を高める。面接で答えられなかった質問が、次のコードを書くときの理解を深める。開発と就活が、螺旋のように絡み合いながら翔を引き上げていた。
...
リリースから二週間後。翔はAdMobのダッシュボードを開いた。
...
翔のコードが、八円を生み出した。翔がCursorと対話しながら書いた@<code>{BannerAdView}、翔が設計した@<code>{InterstitialAdManager}、翔がデザインしたフラッシュカードのUI——それらが組み合わさって、八円という収益が発生した。

デザイン会社では、翔の仕事に対して月給が支払われていた。しかしそれは「会社の仕事」に対する報酬であり、翔個人の作品に対するものではなかった。

八円は、翔の@<b>{名前}で、翔の@<b>{作品}から発生した収益だ。

翔はスクリーンショットを撮り、Xに投稿した。
...
翔はそのすべてに全力で臨んだ。山崎さんのアドバイスを胸に、知っていることは正確に伝え、知らないことは正直に認めた上で、学ぶ姿勢を示した。

面接を重ねるうちに、翔は気づいた。面接官が最も興味を示すのは、技術的な知識の正確さではなく、翔の@<b>{ストーリー}だった。

グラフィックデザイナーからiOSエンジニアへの転身。AIを先生として使った学習法。リジェクトされて再申請してリリースした経験。一人でもくもく会を開催し続けた粘り強さ。海外ハッカソンで失格になりながらもアプリをリリースした実行力。

翔の一年間の旅路そのものが、最強のポートフォリオだった。
...
翔は悩んだ。一社目はエンジニアとしての成長に集中できる環境。二社目はデザイナーの経験を活かせる環境。

翔は山崎さんに相談した。

「雪川さんの場合、最初はエンジニアリングに集中した方がいいと思います。デザインスキルは後からでも活かせます。しかし、エンジニアとしての基礎——チーム開発、コードレビュー、CI/CD、テスト——は実務で経験しないと身につきません」

翔は一社目を選んだ。ヘルスケアアプリのスタートアップ。SwiftUIのiOSエンジニアとして、翌年の二月に入社する。

もくもく会で報告した。

「iOSエンジニア&デザイナーもくもく会 vol.10」。参加者は八名。翔、Haru、常連のメンバーに加え、新しい参加者も増えていた。
...
「もくもく会は続けます」と翔は言った。「入社しても、ここは自分の原点なので」
...
//quote{
@yukikawa_sho おめでとう！1年でここまで来たのは、翔さんの行動力の賜物です。でもHaruさんの言う通り、ここからが本番。実務ではチーム開発、コードレビュー、テスト、CI/CD——個人開発にはなかった壁がたくさんあります。でも翔さんなら大丈夫。困ったらまた聞いてください。
//}

中野にも連絡した。一年前、言語交換ミートアップで出会い、コワーキングスペースに誘ってくれた中野。Ignoreのアイコンを作ることしかできなかった翔を、iOSの世界に導いてくれた人。
...
//quote{
マジで？笑　翔くんすごいな。アイコン作ってもらったときから、デザインの目があるなとは思ってたけど、エンジニアにまでなるとは。今度一緒にアプリ作ろうよ。
//}
...
翔の部屋は、MacBook Airから放たれる光に照らされている。

一年前と同じ時間。同じ部屋。同じMacBook Air。

しかし、その光の色が変わっていた。

一年前の午前一時。画面にはクライアントのデザインデータが映り、冷たい光が翔の顔を照らしていた。壁には貼りっぱなしのポスター、床には読みかけのデザイン雑誌。「これでいいのか」と吐き出した息が、白く濁ってモニターに反射していた。

今の午前一時。画面にはXcodeのプロジェクトが映っている。SwiftWordsのアップデート版。AdMobの最適化とUIの改善。翔の指がキーボードの上を走る。Cursorのチャットウィンドウには、翔の質問とAIの回答が並んでいる。

壁には、App Storeのスクリーンショットをプリントしたものが貼ってある。Kamakura Temple、SwiftLingo、SwiftWords——三つのアプリ。翔のデザイナーとしての目が選んだ構図で撮影された、美しいスクリーンショット。
...
しかし、翔は知っている。この数字の裏には、一年分の物語がある。
...
GitHubのプロフィールページを開いた。十二のリポジトリ。三百以上のコントリビューション。緑色のタイルが、翔がコードを書いた日々を示している。
...
今は、翔の作品を世界に届け、翔に仲間を繋ぎ、翔の人生を変えた道具だ。

翔はベッドに入った。二月の入社まで、あと二ヶ月。その間にやることはたくさんある。SwiftWordsのアップデート。もくもく会の継続開催。来年のShipatonへの準備。そして、チーム開発に向けた勉強——テストコード、CI/CD、コードレビュー。
