...
　蓮はターミナルを開いた。昨夜@<tt>{create-next-app}で生成したプロジェクトが、まだ骨組みだけの状態で待っている。@<tt>{src/app/page.tsx}に書いた「ここから始める」の一行を見つめ、それを消した。
...
　RainbowKitとWagmi。SDKのドキュメントを読み漁っていたときに名前だけは知っていた。RainbowKitはウォレット接続のUIコンポーネント。Wagmiはそれを支えるReactフック集。この二つを組み合わせれば、MetaMaskやWalletConnectなど複数のウォレットに対応した接続画面を、ほとんどゼロから書かずに実装できる。

　蓮はまだ使ったことがない。

　でも、ドキュメントを読む限り、Reactの作法に忠実な設計だった。@<tt>{Provider}でラップし、フックで状態を取り、コンポーネントでUIを出す。蓮の土俵だ。ブロックチェーンの部分だけが未知で、それ以外はいつものReact開発と変わらない。
...
　@<tt>{npm install @rainbow-me/rainbowkit wagmi viem}
...
　次に、アプリ全体をProviderでラップする。RainbowKitが提供する@<tt>{RainbowKitProvider}と、WagmiのQueryClientProviderを重ねる。Reactの世界では「コンテキスト」と呼ばれるパターンだ。木の根元に栄養を注ぐと、枝葉のどのコンポーネントからでもその栄養——この場合はウォレットの接続状態——にアクセスできる。
...
　パソコンの画面が切り替わった。ボタンの文字が「Connect Wallet」からウォレットアドレスの先頭と末尾——@<tt>{0x7a3B...9f2E}——に変わる。
...
　蓮の指が止まったのは、@<tt>{amount}の型を書くときだった。

　直感的には@<tt>{Float}か@<tt>{Int}を使いたくなる。金額なのだから、数値型であるべきだ。しかし第三章で学んだことを思い出す。J-Coinは@<tt>{decimals = 18}。一J-Coinは内部で@<tt>{1000000000000000000}——十の十八乗——として表現される。この巨大な整数をデータベースの通常の数値型に入れると、精度が失われる可能性がある。

　@<tt>{String}。文字列として保存する。見た目は不格好だが、精度は一桁も失われない。お金を扱うとき、「だいたい正しい」は許されない。一円のずれが、信頼を壊す。
...
　十六バイトの暗号学的乱数を生成し、三十二文字の十六進数文字列に変換する。@<tt>{a3f8c1d9e7b2054f6a9d3c8e1b7f4a20}。こんな文字列が、決済リクエストごとに一つずつ割り当てられる。データベースの@<tt>{@unique}制約により、同じノンスを持つ決済リクエストは二度と作れない。
...
　@<tt>{paymentRequestId}に@<tt>{@unique}。一つの決済リクエストに対して、トランザクションは一つだけ。@<tt>{txHash}にも@<tt>{@unique}。ブロックチェーン上の同じトランザクションが、二度記録されることはない。

　二重の鍵。ノンスでリクエストの一意性を守り、トランザクションハッシュで支払いの一意性を守る。ベルトとサスペンダー。どちらか一方が外れても、もう一方が支えてくれる。

　蓮は@<tt>{npx prisma db push}を実行した。スキーマがデータベースに反映される。テーブルが生まれた。まだ空のテーブル。しかしその構造の中に、蓮が考え得るかぎりの安全策が埋め込まれている。
...
　客のウォレットアプリが送金を実行すると、トランザクションハッシュが返される。六十四桁の十六進数。@<tt>{0x}で始まる、世界に一つだけの識別子。そのハッシュを蓮のサーバーに送信し、サーバーがブロックチェーン上のトランザクションを検証する。
...
　さらに、レシートの中のイベントログを解析する。ERC-20の@<tt>{Transfer}イベント。第三章で読んだ@<tt>{emit Transfer(owner, to, amount)}の痕跡が、ここに記録されている。

　送金先アドレスが、決済リクエストの@<tt>{toAddress}と一致するか。金額がリクエストの@<tt>{amount}と一致するか。一つでも食い違えば、その送金はこの決済リクエストに対するものではない。

　検証をパスしたら、@<tt>{PaymentRequest}のステータスを@<tt>{COMPLETED}に、@<tt>{Transaction}レコードを@<tt>{CONFIRMED}で作成する。この二つの操作は同時に成功するか、同時に失敗するかでなければならない。片方だけ成功すると、データの整合性が壊れる。
...
　@<tt>{prisma.$transaction}。二つのデータベース操作をアトミックに実行する。「原子的」——それ以上分割できない一つの塊として扱う。片方が失敗すれば、もう片方もロールバックされる。

　蓮はこのコードを書いたとき、第三章で読んだスマートコントラクトの@<tt>{transfer}関数を思い出した。あの関数も、残高の減算と加算を一つの処理としてまとめていた。「引いて、足して、記録する」。ブロックチェーンの世界でもアプリの世界でも、お金を扱うコードの核心は同じだ。一つの操作が途中で止まってはならない。
...
//quote{
@<b>{fujisaki_ren}: 決済ページで、チェーン切り替えのUIがうまく動かない。ユーザーがPolygon以外のネットワークにいるとき、切り替えを促すんだけど、MetaMaskのポップアップが出ないケースがある。
//}

//quote{
@<b>{MeiHua_Chen}: あー、それ私もハマった！ Wagmiの@<tt>{useSwitchChain}フック使ってる？
//}

//quote{
@<b>{fujisaki_ren}: はい、でもリジェクトされたときのエラーハンドリングが……
//}

//quote{
@<b>{MeiHua_Chen}: @<tt>{UserRejectedRequestError}をキャッチして、「ネットワークをPolygonに切り替えてください」ってメッセージ出すのが一番わかりやすいよ。ユーザーが手動で切り替える場合もあるから、@<tt>{useAccount}の@<tt>{chain}を監視して、正しいチェーンに変わったら自動でUIを更新する。
//}

//quote{
@<b>{fujisaki_ren}: なるほど……ちょっとやってみます。
//}

　十五分後。

//quote{
@<b>{fujisaki_ren}: 動いた！ ありがとう美華さん。
//}

//quote{
@<b>{MeiHua_Chen}: よかった😊 私もクロスボーダーのやつでWagmiのフック全部使い倒してるから、何かあったら聞いて。エコシステムの仲間でしょ。
//}
...
　蓮はテストネットのJ-Coinを用意した。以前フォーセットから取得したテストトークンがまだウォレットに残っている。もう一つ、店舗側のウォレットとしてAccount 2を用意し、テスト用の店舗アカウントをアプリに登録した。
...
　有効期限のカウントダウンが始まった。@<tt>{4:59}、@<tt>{4:58}、@<tt>{4:57}——
...
　蓮が設計したデータベースにノンスが記録され、QRコードが生成され、客のウォレットからJ-Coinが送金され、ブロックチェーン上でトランザクションが承認され、サーバーがその正当性を検証し、ステータスが@<tt>{PENDING}から@<tt>{COMPLETED}に遷移した。蓮が書いたコードの、最初の一行目から最後の一行まで、すべてが意図どおりにつながった。
...
店舗側：金額入力 → QRコード生成 → 決済完了通知客側　：QRスキャン → ウォレット接続 → J-Coin送金
...
　二週間前、このデスクで@<tt>{create-next-app}を叩いたときは、完成する保証などなかった。ブロックチェーン歴一週間の人間がQRコード決済アプリを作る。冷静に考えれば無謀だった。
